...
export const deleteWorkoutController = async (req, resp) => {
    try {
        const { id } = req.params
        ...
        const deleteWorkout = await WorkoutsModel.findByIdAndDelete({ _id: id })
        ...
        resp.json({
            success: true,
            message: "Workout deleted from the database",
            deleteWorkout
        }).status(200)
    } catch (error) {
        resp.json({
            success: false,
            message: "Error in Deleting the Workout",
            error
        }).status(400)
    }
}